...
from . import database, models, config, migration_utils, api_manager, coinlist_manager
...
@app.get("/coinlist_manager")
async def coinlist_manager(request: Request, db: Session = Depends(database.get_db)):
    try:
        # some code to retrieve data from the database and process it
        return templates.TemplateResponse("coinlist_manager.html", {"request": request, "data": processed_data})
    except Exception as e:
        logging.error(f"An error occurred while rendering the coinlist_manager template: {e}")
        raise HTTPException(status_code=500, detail="Internal Server Error")
